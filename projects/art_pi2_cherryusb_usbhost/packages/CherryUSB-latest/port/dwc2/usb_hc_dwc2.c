...
static void dwc2_iso_urb_init(struct usbh_bus *bus, uint8_t chidx, struct usbh_urb *urb)
{
    struct dwc2_chan *chan;
    ...
    iso_packet = &urb->iso_packet[chan->iso_frame_idx];
    ...
    dwc2_chan_init(bus,
                   ...
                   USB_GET_MULT(urb->ep->wMaxPacketSize) + 1,
                   urb->hport->speed);
    dwc2_chan_transfer(bus, chidx, urb->ep->bEndpointAddress, iso_packet->transfer_buffer, chan->xferlen, chan->num_packets, data_pid);
}
    ...
    if (chan_intstatus & USB_OTG_HCINT_CHH) {
        ...
        if (chan_intstatus & USB_OTG_HCINT_XFRC) {
            ...
            if (USB_GET_ENDPOINT_TYPE(urb->ep->bmAttributes) == USB_ENDPOINT_TYPE_CONTROL) {
                ...
                struct usbh_iso_frame_packet *iso_packet = &urb->iso_packet[chan->iso_frame_idx];

                iso_packet->actual_length = count;
                chan->iso_frame_idx++;
                ...
                if (chan->iso_frame_idx == urb->num_of_iso_packets) {
                    ...
                    dwc2_iso_urb_init(bus, ch_num, urb);
                }
            } else {
                if (chan->do_ssplit && urb->transfer_buffer_length > 0 && (count == USB_GET_MAXPACKETSIZE(urb->ep->wMaxPacketSize))) {
