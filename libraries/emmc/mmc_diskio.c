...
static rt_sem_t emmc_sem = RT_NULL;
...
/* Private functions ---------------------------------------------------------*/
static DSTATUS MMC_CheckStatus(uint8_t lun)
{
  Stat = STA_NOINIT;
//  while (HAL_HSEM_FastTake(EMMC_HSEM_ID) != HAL_OK)
//  {
//  }
  rt_sem_take(emmc_sem, RT_WAITING_FOREVER);

  if((BSP_MMC_GetCardState(0) == BSP_ERROR_NONE))
  {
    Stat &= ~STA_NOINIT;
  }
  rt_sem_release(emmc_sem);
...
//  {
//  }
  emmc_sem = rt_sem_create("emmc_sem", 0, RT_IPC_FLAG_PRIO);
//  rt_sem_take(emmc_sem, RT_WAITING_FOREVER);
...
/**
  ...
  * @retval DSTATUS: Operation status
  */
DSTATUS MMC_status(uint8_t lun)
{
...
DSTATUS MMC_getCardInfo(uint8_t lun)
{
  ...
  if((BSP_MMC_GetCardCID(0, &card_cid)== BSP_ERROR_NONE))
  {
      Stat &= ~STA_NOINIT;
  }
   return Stat;
