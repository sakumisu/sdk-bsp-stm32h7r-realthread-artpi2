/**
  ...
  * @brief   This file includes a standard driver for the APS256XX 16bits-HSPI memory
  *          mounted on HSPI/OSPI daughter board MB1648 used with PEB3 Cobra MB1309 board.
  ...
  ==============================================================================
  [..]
   (#) This driver is used to drive the APS256XX 16bits-HSPI PSRAM external memory mounted on
       on HSPI/OSPI daughter board MB1648 used with PEB3 Cobra MB1309 board.

   (#) This driver need specific component driver (APS256XX) to be included with.

   (#) APS256XX Initialization steps:
   ...
   (#) APS256XX 16bits-HSPI PSRAM memory operations
...
int32_t BSP_HSPI_RAM_Init(uint32_t Instance, BSP_HSPI_RAM_Cfg_t *Init)
{
  ...
  else
  {
    ...
    if (Hspi_Ram_Ctx[Instance].IsInitialized == HSPI_ACCESS_NONE)
    {
      ...
      hspi_init.MemorySize     = (uint32_t)POSITION_VAL(APS256XX_RAM_SIZE);
      hspi_init.SampleShifting = HAL_XSPI_SAMPLE_SHIFT_HALFCYCLE;
      ...
      /* Reset the memory */
      else if (APS256XX_Reset(&hhspi_ram[Instance]) != 0)/*APS256XX_OK*/
      {
...
__weak HAL_StatusTypeDef MX_HSPI_RAM_Init(XSPI_HandleTypeDef *hhspi, MX_HSPI_InitTypeDef *Init)
{
  ...
  hhspi->Init.DelayHoldQuarterCycle   = HAL_XSPI_DHQC_DISABLE;  // HAL_XSPI_DHQC_ENABLE
  hhspi->Init.ChipSelectBoundary      = HAL_XSPI_BONDARYOF_16KB; // HAL_XSPI_BONDARYOF_NONE, HAL_XSPI_BONDARYOF_16KB
  ...
  hhspi->Init.MemorySelect = HAL_XSPI_CSSEL_NCS1;
...
int32_t BSP_HSPI_RAM_Read(uint32_t Instance, uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
  ...
  {
    if (APS256XX_Read(&hhspi_ram[Instance], pData, ReadAddr, Size,
                      (uint32_t)APS256XX_READ_LATENCY((uint32_t)(Hspi_Ram_Ctx[Instance].ReadLatencyCode),
                                                      (uint32_t)(Hspi_Ram_Ctx[Instance].LatencyType)),
                      (uint32_t)(Hspi_Ram_Ctx[Instance].IOMode),
                      (uint32_t)(Hspi_Ram_Ctx[Instance].BurstType)) != (int32_t)APS256XX_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
    else
...
int32_t BSP_HSPI_RAM_Read_DMA(uint32_t Instance, uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
  ...
  {
    if (APS256XX_Read_DMA(&hhspi_ram[Instance], pData, ReadAddr, Size,
                          (uint32_t)APS256XX_READ_LATENCY((uint32_t)(Hspi_Ram_Ctx[Instance].ReadLatencyCode),
                                                          (uint32_t)(Hspi_Ram_Ctx[Instance].LatencyType)),
                          (uint32_t)(Hspi_Ram_Ctx[Instance].IOMode),
                          (uint32_t)(Hspi_Ram_Ctx[Instance].BurstType)) != (int32_t)APS256XX_OK)
...
int32_t BSP_HSPI_RAM_Write(uint32_t Instance, uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  {
    if (APS256XX_Write(&hhspi_ram[Instance], pData, WriteAddr, Size,
                       (uint32_t)APS256XX_WRITE_LATENCY((uint32_t)(Hspi_Ram_Ctx[Instance].WriteLatencyCode)),
                       (uint32_t)(Hspi_Ram_Ctx[Instance].IOMode),
                       (uint32_t)(Hspi_Ram_Ctx[Instance].BurstType)) != (int32_t)APS256XX_OK)
    {
      ret = BSP_ERROR_COMPONENT_FAILURE;
...
int32_t BSP_HSPI_RAM_Write_DMA(uint32_t Instance, uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  {
    if (APS256XX_Write_DMA(&hhspi_ram[Instance], pData, WriteAddr, Size,
                           (uint32_t)APS256XX_WRITE_LATENCY((uint32_t)(Hspi_Ram_Ctx[Instance].WriteLatencyCode)),
                           (uint32_t)(Hspi_Ram_Ctx[Instance].IOMode),
                           (uint32_t)(Hspi_Ram_Ctx[Instance].BurstType)) != (int32_t)APS256XX_OK)
    {
...
int32_t BSP_HSPI_RAM_EnableMemoryMappedMode(uint32_t Instance)
{
  ...
  {
    if (APS256XX_EnableMemoryMappedMode(&hhspi_ram[Instance], (uint32_t)APS256XX_READ_LATENCY((uint32_t)\
                                        ((uint32_t)(Hspi_Ram_Ctx[Instance].ReadLatencyCode)),
                                        (uint32_t)(Hspi_Ram_Ctx[Instance].LatencyType)),
                                        APS256XX_WRITE_LATENCY((uint32_t)\
  ...
  else
  {
    ...
    else
    {
      ...
      if (ret == BSP_ERROR_NONE)
      {
        ...
        MODIFY_REG(hhspi_ram[Instance].Instance->CR, (XSPI_CR_FMODE), 0U);
        Hspi_Ram_Ctx[Instance].IsInitialized = HSPI_ACCESS_INDIRECT;
      }
    }
  }
...
int32_t BSP_HSPI_RAM_Config16BitsOctalRAM(uint32_t Instance, BSP_HSPI_RAM_Cfg_t *Cfg)
{
  ...
  /* Check if the instance is supported */
  if ((Instance >= HSPI_RAM_INSTANCES_NUMBER)
      || (Cfg->BurstLength == APS256XX_BURST_2_KBYTES))
  {
    ret = BSP_ERROR_WRONG_PARAM;
  }
  else
  {
    ...
    else
    {
          ...
          (Hspi_Ram_Ctx[Instance].ReadLatencyCode != Cfg->ReadLatencyCode))
      {
        ...
        if (APS256XX_ReadReg(&hhspi_ram[Instance], APS256XX_MR0_ADDRESS, reg,
                             (uint32_t)APS256XX_READ_REG_LATENCY((uint32_t)(Hspi_Ram_Ctx[Instance].ReadLatencyCode)))\
            != (int32_t)APS256XX_OK)
        {
          ret = BSP_ERROR_COMPONENT_FAILURE;
        }
        else
        {
          ...
          if (APS256XX_WriteReg(&hhspi_ram[Instance], APS256XX_MR0_ADDRESS, reg[0]) != APS256XX_OK)
          {
            ret = BSP_ERROR_COMPONENT_FAILURE;
          }
      ...
      if (ret == BSP_ERROR_NONE)
      {
            ...
            (Hspi_Ram_Ctx[Instance].WriteLatencyCode != Cfg->WriteLatencyCode))
        {
          ...
          if (APS256XX_ReadReg(&hhspi_ram[Instance], APS256XX_MR4_ADDRESS, reg,
                               (uint32_t)APS256XX_READ_REG_LATENCY((uint32_t)(Hspi_Ram_Ctx[Instance].ReadLatencyCode)))\
              != (int32_t)APS256XX_OK)
          {
            ret = BSP_ERROR_COMPONENT_FAILURE;
          ...
          else
          {
            ...
            if (APS256XX_WriteReg(&hhspi_ram[Instance], APS256XX_MR4_ADDRESS, reg[0]) != APS256XX_OK)
            {
              ret = BSP_ERROR_COMPONENT_FAILURE;
            }
          }
      ...
      if (ret == BSP_ERROR_NONE)
      {
            ...
            (Hspi_Ram_Ctx[Instance].IOMode         != Cfg->IOMode))
        {
          ...
          if (APS256XX_ReadReg(&hhspi_ram[Instance], APS256XX_MR8_ADDRESS, reg,
          ...
          {
            /* Configure the 16-bits Octal RAM memory ***************************/
            MODIFY_REG(reg[0], ((uint8_t)APS256XX_MR8_X8_X16 | (uint8_t)APS256XX_MR8_BL),
                       ((uint8_t)(Cfg->IOMode) | (uint8_t)(Cfg->BurstLength)));

            if (APS256XX_WriteReg(&hhspi_ram[Instance], APS256XX_MR8_ADDRESS, reg[0]) != APS256XX_OK)
            {
              ret = BSP_ERROR_COMPONENT_FAILURE;
            }
            else
...
int32_t BSP_HSPI_RAM_EnterDeepPowerDown(uint32_t Instance)
{
  ...
  else if (APS256XX_EnterPowerDown(&hhspi_ram[Instance]) != APS256XX_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
...
int32_t BSP_HSPI_RAM_LeaveDeepPowerDown(uint32_t Instance)
{
  ...
  else if (APS256XX_LeavePowerDown(&hhspi_ram[Instance]) != APS256XX_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
...
static void HSPI_RAM_MspInit(XSPI_HandleTypeDef *hhspi)
{
  ...
  hhspi->Init.MemoryMode = HAL_XSPI_SINGLE_MEM;
  hhspi->Init.MemoryType = HAL_XSPI_MEMTYPE_APMEM_16BITS;
  hhspi->Init.MemorySize = HAL_XSPI_SIZE_32GB;
  hhspi->Init.ChipSelectHighTimeCycle = 1U;
  hhspi->Init.FreeRunningClock = HAL_XSPI_FREERUNCLK_DISABLE;
  hhspi->Init.ClockMode = HAL_XSPI_CLOCK_MODE_0;
  hhspi->Init.WrapSize = HAL_XSPI_WRAP_NOT_SUPPORTED;
  hhspi->Init.ClockPrescaler = 0;
  hhspi->Init.SampleShifting = HAL_XSPI_SAMPLE_SHIFT_NONE;
  hhspi->Init.DelayHoldQuarterCycle = HAL_XSPI_DHQC_ENABLE;
  hhspi->Init.ChipSelectBoundary = HAL_XSPI_BONDARYOF_8KB;
  hhspi->Init.MaxTran = 0;
  hhspi->Init.Refresh = ((2U * (xspi1_clk / (hhspi->Init.ClockPrescaler + 1U))) / 1000000U) - 4U;;
  hhspi->Init.MemorySelect = HAL_XSPI_CSSEL_NCS1;
  ...
  if (HAL_XSPIM_Config(hhspi, &sXspiManagerCfg, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
  {
    Error_Handler();
  }
...
static void HSPI_RAM_MspDeInit(const XSPI_HandleTypeDef *hhspi)
{
  ...
  // UNUSED(hhspi);
  HAL_XSPI_DeInit((XSPI_HandleTypeDef *)hhspi);
